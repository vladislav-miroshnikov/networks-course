...
1. **Время передачи пакета**  
   ...
   $$
   T_{\text{trans}} = \frac{12000}{10^9} = 1.2 \times 10^{-5} \, \text{с} = 0.012 \, \text{мс}.
   $$

2. **Время оборота (RTT)**  
   Дано: \( \text{RTT} = 30 \, \text{мс} = 0.03 \, \text{с} \).  
   Так как \( T_{\text{trans}} \) мало по сравнению с RTT, им можно пренебречь.

3. **Использование канала**  
   Фактическая пропускная способность:
   $$
   T \approx \frac{n \times 12000}{\text{RTT}}.
   $$  
   Использование канала:
   $$
   U = \frac{T}{R} = \frac{n \times 12000}{\text{RTT} \times R}.
   $$  
   Требуется: \( U > 0.98 \).

4. **Подстановка значений**
   $$
   \frac{n \times 12000}{0.03 \times 10^9} > 0.98.
   $$  
   Упростим:
   $$
   \frac{n \times 12000}{3 \times 10^7} = \frac{n}{2500} > 0.98.
   $$

5. **Решение неравенства**
   $$
   n > 0.98 \times 2500 = 2450.
   $$  
   Так как \( n \) — целое число, минимальное значение: \( n = 2451 \).

6. **Проверка**  
   При \( n = 2451 \):
   $$
   U = \frac{2451}{2500} = 0.9804 > 0.98.
   $$  
   Условие выполнено.

#### Ответ  
Размер окна \( n \) должен быть не менее **2451**.
