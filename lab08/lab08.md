...
#### Решение
### Шаг 1: Моделирование увеличения окна

Предположим, что увеличение размера окна происходит каждое RTT (round-trip time), так как это соответствует типичному поведению TCP, где размер окна обновляется после получения подтверждений за полный цикл. Начальный размер окна $W_0 = \frac{W}{2}$. После каждого RTT окно увеличивается:

\[ $W_{t+1} = W_t \times (1 + a)$ \]

После $n$ RTT размер окна становится:

\[ $W_n = W_0 \times (1 + a)^n = \frac{W}{2} \times (1 + a)^n$ \]

Нужно найти $n$, при котором $W_n = W$:

\[ $\frac{W}{2} \times (1 + a)^n = W$ \]

\[ $(1 + a)^n = 2$ \]

\[ $n = \frac{\ln 2}{\ln (1 + a)}$ \]

Число $n$ — это количество RTT, необходимое для увеличения окна с $\frac{W}{2}$ до $W$. Поскольку $a$ — константа ($0 < a < 1$), $n$ не зависит от $W$ или пропускной способности соединения

### Шаг 2: Количество отправленных пакетов в цикле

В TCP окно перегрузки определяет, сколько пакетов отправляется за одно RTT. Предположим, что в каждом RTT отправляется количество пакетов, равное текущему размеру окна в начале RTT. Цикл начинается с $W_0 = \frac{W}{2}$, увеличивается до $W$, после чего происходит потеря, и окно сбрасывается до $\frac{W}{2}$.

Размер окна в $i$-м RTT:

\[ $W_i = \frac{W}{2} \times (1 + a)^i$ \]

Мы предполагаем, что потеря происходит, когда окно достигает $W$, то есть в RTT номер $n$. Общее количество пакетов $P$, отправленных за цикл (от $i = 0$ до $i = n-1$), до достижения $W$:

\[ $P = \sum_{i=0}^{n-1} W_i = \sum_{i=0}^{n-1} \frac{W}{2} \times (1 + a)^i $ \]

Это геометрическая сумма:

\[ $\sum_{i=0}^{n-1} (1 + a)^i = \frac{(1 + a)^n - 1}{(1 + a) - 1} = \frac{(1 + a)^n - 1}{a}$ \]

таккак $(1 + a)^n = 2$:

\[ $\sum_{i=0}^{n-1} (1 + a)^i = \frac{2 - 1}{a} = \frac{1}{a}$ \]

Тогда:

\[ $P = \frac{W}{2} \times \frac{1}{a} = \frac{W}{2a}$ \]

Итак, за один цикл отправляется примерно $\frac{W}{2a}$ пакетов.

### Шаг 3: Связь с частотой потерь $L$

Частота потерь $L$ — это вероятность потери пакета, или доля потерянных пакетов. Предположим, что в каждом цикле происходит одна потеря, когда окно достигает максимального значения $W$. Тогда $L$ равна обратной величине количества пакетов, отправленных за цикл:

\[ $L = \frac{\text{число потерь}}{\text{число пакетов}} = \frac{1}{P} = \frac{1}{\frac{W}{2a}} = \frac{2a}{W}$ \]

Значит мы получили функциональную зависимость:

\[ $L = \frac{2a}{W}$ \]

Или, выражая $W$ через $L$:

\[ $W = \frac{2a}{L}$ \]

### Шаг 4: Проверка результата

- **Зависимость**: $L \propto \frac{1}{W}$: чем больше максимальное окно $W$, тем реже происходят потери.
- **Константа $a$**: Поскольку $0 < a < 1$, коэффициент $2a$ — положительная константа, зависящая от параметра протокола.
- **Сравнение со стандартным TCP**: В стандартном TCP с аддитивным увеличением и multiplicative decrease зависимость $W \propto \frac{1}{\sqrt{L}}$, но здесь, из-за мультипликативного увеличения, зависимость становится $W \propto \frac{1}{L}$.

### Итоговый ответ

Функциональная зависимость между частотой потерь $L$ и максимальным размером окна $W$:
